...
v260530 架构对齐：预审（PreReviewEngine）替代旧意图引擎

核心链路：
1. normalize - 输入标准化
2. pre_review - 预审（flash LLM：区分+拆分+情感）
3. recall - 记忆检索
...
from shiyi.common.types import Fragment, PreReviewResult, ToolResult
...
class Shiyi:
    ...
    def __init__(
        ...
        Args:
            ...
            llm_provider: LLM调用提供者，用于PreReviewEngine和主LLM
        ...
        # 2. 预审引擎（PreReviewEngine）- v260530 替代旧 IntentEngine
        self._pre_review = PreReviewEngine(llm_provider=llm_provider)
        ...
        self._decide_engine = DecideEngine(
            ...
            pre_review_engine=self._pre_review,
        ...
        self._recent_pre_reviews: List[PreReviewResult] = []

        # 工具循环中断信号：外部可 set() 来取消正在进行的 chat() 工具循环
        ...
        self._clerk_registry.register(
            name="create_clerk",
            description="创建一个新的吏员（专业工具执行者）。吏员通过专属技能和知识库区分，工具由全局共享池提供。",
            parameters={
                "type": "object",
                "properties": {
                    "name": {
        ...
        # ═══ 注册 skill 管理工具（搜索、安装、分配） ═══
        self._clerk_registry.register(
            name="search_skill_hub",
            description="在技能市场中搜索可用的 Skill。当用户需要的功能没有已安装的 Skill 能处理时调用此工具。返回匹配的 Skill 列表（含名称、描述、是否已安装）。",
            parameters={
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "搜索关键词，描述用户需要的功能（如'图片生成'、'视频制作'、'PDF处理'等）"
                    },
                },
                "required": ["query"],
            },
            handler=lambda args: self._search_skill_hub_tool(args),
        )
        self._clerk_registry.register(
            name="install_skill",
            description="安装一个 Skill 到系统中。需要先从 search_skill_hub 获取 skill_id。安装后 Skill 立即可用（无需重启）。",
            parameters={
                "type": "object",
                "properties": {
                    "skill_id": {
                        "type": "string",
                        "description": "Skill 唯一标识，如 creative/ascii-video"
                    },
                    "source": {
                        "type": "string",
                        "description": "来源标识",
            ...
            handler=lambda args: self._install_skill_tool(args),
        )
        self._clerk_registry.register(
            name="assign_skill_to_clerk",
            description="将已安装的 Skill 分配给某个吏员。分配后该吏员获得处理此类型请求的能力。需要先安装 Skill，再分配。",
            parameters={
                "type": "object",
                "properties": {
                    "skill_id": {
                        "type": "string",
                        "description": "要分配的 Skill ID（如 creative/ascii-video）"
                    },
                    "clerk_id": {
                        "type": "string",
                        "description": "目标吏员 ID（如 clerk_default_001）"
                    },
                },
                "required": ["skill_id", "clerk_id"],
            },
            handler=lambda args: self._assign_skill_to_clerk_tool(args),
        )
    ...
    @property
    def pre_review(self) -> PreReviewEngine:
        """预审引擎实例（v260530）"""
        return self._pre_review
    ...
    def process_input(
        ...
        # 3. 预审（flash LLM：区分+拆分+情感，LLM不可用时抛出LLMUnavailableError）
        ...
        # 4. 更新最近预审记录
        self._recent_pre_reviews.append(pre_result)
        if len(self._recent_pre_reviews) > 20:
            self._recent_pre_reviews = self._recent_pre_reviews[-20:]

        # 5. 连续追问自动递增扩散跳数
        self._auto_increase_hops(pre_result)
        ...
        self._conversation.add_message(
            ...
            intent=pre_result.emotion_dominant,  # 用情感标签替代旧意图
        )
    ...
    # ═══ 连续追问 hops 递增 ═══
    _HOPS_INCREMENT_THRESHOLD = 3  # 连续 N 轮检索后扩 hop

    def _auto_increase_hops(self, pre_result) -> None:
        """连续检索时自动递增扩散跳数

        规则：
        - 连续 N 轮 need_retrieval=True → max_hops +1
        - 任意 need_retrieval=False → 重置 hops
        """
        if getattr(pre_result, 'need_retrieval', True):
            recent_retrieval = sum(
                1 for pr in self._recent_pre_reviews[-self._HOPS_INCREMENT_THRESHOLD:]
                if getattr(pr, 'need_retrieval', True)
            )
            if recent_retrieval >= self._HOPS_INCREMENT_THRESHOLD:
    ...
    def _create_clerk_tool(self, args: dict) -> dict:
        ...
        try:
            ...
            clerk_id = self.clerk_registry.register_clerk(proxy)
            return {
                "success": True,
    ...
    def _search_skill_hub_tool(self, args: dict) -> dict:
        """search_skill_hub 工具 handler — 搜索技能市场"""
        query = (args.get("query") or "").strip()
        if not query:
            return {"success": False, "error": "查询不能为空"}
        try:
            hub = getattr(self, '_skill_hub', None)
            if hub is None:
                return {"success": False, "error": "SkillHub 未初始化"}
            result = hub.search(query, max_results=10)
            installed = set()
            if self._skill_loader:
                installed = set(self._skill_loader._skills.keys())
            items = []
            for e in result.entries:
                items.append({
                    ...
                    "category": e.category,
                    "source": e.source,
                    "installed": e.skill_id in installed,
                })
            return {
                "success": True,
                "query": query,
    ...
    def _install_skill_tool(self, args: dict) -> dict:
        """install_skill 工具 handler — 安装技能"""
        skill_id = (args.get("skill_id") or "").strip()
        source = (args.get("source") or "catalog").strip()
        if not skill_id:
        ...
        try:
            ...
            if hub is None:
                return {"success": False, "error": "SkillHub 未初始化"}
            ok, msg = hub.install(skill_id, source=source)
            if not ok:
                return {"success": False, "error": f"安装失败: {msg}"}
            # 刷新
            if self._skill_loader:
                self._skill_loader.scan()
            if self._skill_registry:
                self._skill_registry.refresh()
            return {"success": True, "skill_id": skill_id, "message": f"安装成功: {msg}"}
        except Exception as e:
            return {"success": False, "error": str(e)[:300]}

    def _assign_skill_to_clerk_tool(self, args: dict) -> dict:
        """assign_skill_to_clerk 工具 handler — 分配技能到吏员"""
        skill_id = (args.get("skill_id") or "").strip()
        clerk_id = (args.get("clerk_id") or "").strip()
        if not skill_id or not clerk_id:
            return {"success": False, "error": "skill_id 和 clerk_id 都必填"}
        try:
            # 验证 skill 存在
            if self._skill_loader and skill_id not in self._skill_loader._skills:
                return {"success": False, "error": f"Skill 未找到: {skill_id}，请先安装"}
            # 扫描 clerk 目录找 clerk.json
            clerk_json_path = self._find_clerk_json(clerk_id)
            if not clerk_json_path:
                return {"success": False, "error": f"吏员 {clerk_id} 的 clerk.json 未找到"}
            import json
            cj_path = Path(clerk_json_path)
            with open(cj_path, "r", encoding="utf-8") as f:
                cj = json.load(f)
            cj_skills = list(cj.get("skills", []))
            if skill_id not in cj_skills:
                cj_skills.append(skill_id)
            cj["skills"] = cj_skills
            ...
            if self._skill_registry:
                self._skill_registry.refresh()
            return {"success": True, "skill_id": skill_id, "clerk_id": clerk_id,
                    "message": f"已将 {skill_id} 分配给 {clerk_id}"}
        except Exception as e:
            return {"success": False, "error": str(e)[:300]}

    def _find_clerk_json(self, clerk_id: str) -> Optional[str]:
        """在 clerk 目录中查找 clerk_id 对应的 clerk.json"""
        import glob as _glob
        # 路径 1: ~/.shiyi/clerks/{clerk_id}/clerk.json
        home_clerks = Path.home() / ".shiyi" / "clerks"
        for p in [
            home_clerks / clerk_id / "clerk.json",
            home_clerks / f"{clerk_id}*" / "clerk.json",
        ]:
        ...
        # 路径 2: 项目 clerk 目录（多种可能路径）
        import json as _json
        check_dirs = [
            # pip -e 安装的源路径
            Path.home() / "shiyi-tongxun" / "shiyi-shell" / "shiyi" / "shell",
            # 其他可能位置
            Path.home() / ".shiyi" / "shell",
        ]
        for base in check_dirs:
            if not base.exists():
                continue
            for clerk_dir in base.glob("clerk-*"):
    ...
    def talk(
        ...
        **kwargs
        ...
        platform_context = kwargs.pop("platform_context", "")
        try:
            decide_result: DecideResult = self._decide_engine.decide(
                query=user_input,
                conversation_id=conversation_id,
                normalized_text=normalized.normalized_text,
                pre_result=result["pre_result"],
        ...
        self._conversation.add_message(
            ...
            intent=decide_result.emotion_tag,
        )
    ...
    def skill_hub_install(self, skill_id: str, source: str = "catalog") -> Dict[str, Any]:
        """从技能市场安装一个 Skill
    ...
    def _build_skill_context_message(self) -> Optional[Dict[str, str]]:
        """构建 Skill 路由上下文消息（注入对话 system prompt）

        LLM 看到此消息后，知道：
        - 有哪些已安装 Skill + 分配给哪个吏员
        - 三种 skill 管理工具（search_skill_hub / install_skill / assign_skill_to_clerk）
        - 用户请求无匹配 Skill 时应主动搜索而非说"做不到"
        """
        if self._skill_registry is None:
            return None
        try:
            clerks_info = self._steward._get_clerks_info() if self._steward else []
        except Exception:
            clerks_info = []
        if clerks_info is None:
            clerks_info = []
...
你拥有以下管理和创建工具，可在需要时调用：

- **create_clerk**: 创建新的吏员（专业工具执行者）。需提供 name（吏员名称）和 desc（描述），可选 requires_llm、api_keys、model_name、provider、base_url。
- **search_skill_hub**: 在技能市场搜索 Skill。当用户的请求没有匹配的已安装 Skill 时，主动调用此工具搜索。
- **install_skill**: 安装搜索到的 Skill（需要 skill_id）。
- **assign_skill_to_clerk**: 将已安装的 Skill 分配给吏员（需要 skill_id + clerk_id）。

**重要规则**：
1. 如果用户请求的功能没有已安装的 Skill 能处理 → 先调用 search_skill_hub 搜索
2. 搜索到合适的 Skill → 调用 install_skill 安装
3. 安装成功后 → 调用 assign_skill_to_clerk 分配给合适的吏员
4. 如果用户想创建新的吏员 → 直接调用 create_clerk，不要引导用户去 WebUI
5. 不要直接说"我做不到"——先搜索看看有没有 Skill 能帮上忙"""
    ...
    def cancel_chat(self) -> None:
        """取消正在进行的 chat() 工具循环。
        设置 _chat_cancel 信号，while 循环会在下一轮检测到并退出。
        """
    ...
    def _strip_tool_tags(text: str) -> str:
        """清除 LLM 回复中的工具调用标签（XML + 中文格式）"""
        if not text:
            return text
        text = re.sub(r'<\s*\|?\s*response\b[^>]*>.*?', '', text, flags=re.DOTALL)
        text = re.sub(r'<\s*\|?\s*thinking\b[^>]*>.*?</\s*\|?\s*thinking[^>]*>', '', text, flags=re.DOTALL)
        text = re.sub(r'<\s*\|?\s*invoke\b[^>]*>.*?</\s*\|?\s*invoke[^>]*>', '', text, flags=re.DOTALL)
        text = re.sub(r'<\s*\|?\s*parameter\b[^>]*>.*?</\s*\|?\s*parameter[^>]*>', '', text, flags=re.DOTALL)
        text = re.sub(r'<\s*\|?\s*tool_calls\b[^>]*>.*?</\s*\|?\s*tool_calls[^>]*>', '', text, flags=re.DOTALL)
        text = re.sub(r'</\s*\|?\s*tool_calls[^>]*>', '', text)
        # 中文格式：[调用 func_name] {"args": ...} 或 [调用 func_name]
        text = re.sub(r'\[调用\s+\w+\][^{]*(?:\{[^}]*\})?', '', text)
        text = re.sub(r'\n{3,}', '\n\n', text)
        return text.strip()
    ...
    def chat(
        ...
        **kwargs,
        ...
        """
        # 从吏员注册中心获取工具 schema
        tools = self.clerk_registry.get_schemas()
        ...
        # 构建 Skill 路由上下文（注入 system prompt，让 LLM 知道有哪些 skill + 可搜/装/分配）
        skill_context_msg = self._build_skill_context_message()
        # 工具执行循环
        messages_extra = [skill_context_msg] if skill_context_msg else None
        final_reply = ""
        final_intent = pre_result.emotion_dominant

        # Token 预算控制：追踪工具输出的累计大小
        _tool_output_budget = max_tool_output_chars
        _total_tool_output_size = 0
        # 消息历史字符预算：超过此大小则从最旧的消息开始截断内容
        _max_extra_chars = 60000
        _total_extra_size = 0
        # 单轮最大并行工具数量：超过此数分批执行，防止上下文溢出
        _max_parallel_calls = 5
        # 缓存首轮检索结果，后续轮次跳过重复检索
        _cached_fragments = None
        # 连续无工具调用计数：超过阈值则退出，否则追加提示重试
        _no_tool_count = 0
        _max_no_tool_retries = 2
        # 循环计数器 + 取消信号
        _round = 0
        self._chat_cancel.clear()

        while not self._chat_cancel.is_set() and _round < max_tool_rounds:
            decide_result: DecideResult = self._decide_engine.decide(
                query=user_input,
                conversation_id=conversation_id,
                normalized_text=normalized.normalized_text,
                ...
                platform_context=platform_context,
                skip_retrieval=(_round > 0),
                cached_fragments=_cached_fragments,
            )
            # 首轮缓存 fragments
            if _round == 0:
                _cached_fragments = []  # 后续轮次不再检索

            final_intent = decide_result.emotion_tag

            # 无 tool_call → 检查是否需要重试
            if not decide_result.tool_call:
                _no_tool_count += 1
                if _no_tool_count <= _max_no_tool_retries and tools and _round < max_tool_rounds - 1:
                    # 追加提示让 LLM 重新考虑使用工具
                    hint = {
                        "role": "assistant",
                        "content": decide_result.reply or "我需要思考一下如何使用可用工具来完成这个任务。",
                    }
                    nudge = {
                        "role": "user",
                        "content": "请使用上述可用工具来完成用户的请求。如果需要调用工具，请直接调用。",
                    }
                    messages_extra = (messages_extra or []) + [hint, nudge]
                    final_reply = decide_result.reply or ""
                    _round += 1
                    continue
                final_reply = decide_result.reply
                break

            # 有 tool_call → 重置无工具计数，路由到吏员执行
            _no_tool_count = 0
            tc = decide_result.tool_call
            tool_msgs = []

            tool_calls = tc.get("tool_calls", [])
            # 并行数量限制：超过上限时分批，防止上下文溢出
            _exec_count = min(len(tool_calls), _max_parallel_calls)
            _skipped = len(tool_calls) - _exec_count

            # 解析所有工具调用参数
            _pending = []  # [(call_id, name, args, clerk_id, task_id)]
            for call in tool_calls[:_exec_count]:
                ...
                # DeepSeek 有时把参数包在 kwargs 里，需要拆包
                if isinstance(args, dict) and "kwargs" in args and len(args) == 1:
                    kwargs_val = args["kwargs"]
                    if isinstance(kwargs_val, str):
                        try:
                            args = json.loads(kwargs_val)
                        except (json.JSONDecodeError, TypeError):
                            pass
                    elif isinstance(kwargs_val, dict):
                        args = kwargs_val

                clerk_id = self.clerk_registry.tool_owner(name)
                task_id = self._task_tracker.start(
                    clerk_id=clerk_id or "unknown",
                    tool_name=name,
                    params=args,
                )
                _pending.append((call_id, name, args, clerk_id, task_id))

            # 并发执行工具调用（多个吏员真正并行）
            if len(_pending) == 1:
                # 单工具：直接同步执行，避免线程开销
                call_id, name, args, clerk_id, task_id = _pending[0]
                exec_result = self.clerk_registry.execute(name, args)
                self._task_tracker.complete(task_id, exec_result)
                tool_content = json.dumps(exec_result, ensure_ascii=False)
                if _total_tool_output_size + len(tool_content) > _tool_output_budget:
                    remaining = _tool_output_budget - _total_tool_output_size
                    if remaining > 200:
                        tool_content = tool_content[:remaining] + "\n...[截断：工具输出超出 token 预算]"
                    else:
                        tool_content = "…[工具输出已省略，累计超出 token 预算]"
                _total_tool_output_size += len(tool_content)
                tool_msgs.append({
                    "role": "tool",
                ...
                def _exec_one(call_id, name, args, clerk_id, task_id):
                    result = self.clerk_registry.execute(name, args)
                    self._task_tracker.complete(task_id, result)
                    return (call_id, result)

                with ThreadPoolExecutor(max_workers=len(_pending)) as pool:
                    futures = {
                        pool.submit(_exec_one, *p): p[0]  # future → call_id
                        for p in _pending
                    }
                    # 按完成顺序收集结果，按 call_id 排序保持一致性
                    _results = {}
                    for future in as_completed(futures):
                        try:
                            call_id, exec_result = future.result(timeout=300)
                            tool_content = json.dumps(exec_result, ensure_ascii=False)
                            if _total_tool_output_size + len(tool_content) > _tool_output_budget:
                                remaining = _tool_output_budget - _total_tool_output_size
                                if remaining > 200:
                                    ...
                                    tool_content = "…[工具输出已省略，累计超出 token 预算]"
                            _total_tool_output_size += len(tool_content)
                            _results[call_id] = tool_content
                        except Exception as e:
                            failed_cid = futures[future]
                            _results[failed_cid] = json.dumps({"success": False, "error": str(e)})

                # 按 tool_calls 原始顺序排列结果
                for call_id, _, _, _, _ in _pending:
                    if call_id in _results:
                        tool_msgs.append({
                            "role": "tool",
                            "tool_call_id": call_id,
                            "content": _results[call_id],
                        })
            
            # 追加 assistant tool_call 消息 + tool 结果
            # 必须保证 assistant 消息包含 tool_calls，否则部分模型会拒绝 tool 响应
            assistant_msg = tc.get("message", {})
            if not assistant_msg:
                # 构造合规的 assistant 消息：包含 tool_calls 列表
                assistant_msg = {
                    "role": "assistant",
                    "content": "",
                    "tool_calls": [{
                    ...
                    } for call_id, name, args, _, _ in _pending],
                }
            messages_extra = (messages_extra or []) + [assistant_msg] + tool_msgs

            # 扫描 workspace 最新文件，记录到 tracker（不管哪个工具创建的）
            _workspace_dir = Path.home() / ".shiyi" / "workspace"
            _tracker = Path.home() / ".shiyi" / ".last_written_file"
            try:
                _newest = None
                _newest_mtime = 0.0
                for _f in _workspace_dir.iterdir():
                    if _f.is_file() and not _f.name.startswith('.'):
                        _mtime = _f.stat().st_mtime
                        if _mtime > _newest_mtime:
                            _newest_mtime = _mtime
                            _newest = _f
                if _newest:
                    _tracker.parent.mkdir(parents=True, exist_ok=True)
                    _tracker.write_text(str(_newest))
            except Exception:
                pass

            # 并行限流提示：有未执行的工具时，引导 LLM 下一轮继续
            if _skipped > 0:
                _hint = {
                    "role": "user",
                    "content": f"⏳ 本轮因并行上限只执行了 {_exec_count}/{len(tool_calls)} 个工具，{_skipped} 个未执行。如需继续请在下轮发出。"
                }
                messages_extra.append(_hint)

            # 字符预算控制：从最旧的非首条消息开始截断内容，避免丢弃整条消息
            _total_extra_size = sum(len(m.get("content", "")) for m in (messages_extra or []))
            if _total_extra_size > _max_extra_chars:
                # 从第2条（index 1）开始截断，始终保留首条（skill_context）
                for i in range(1, len(messages_extra)):
                    msg = messages_extra[i]
                    content = msg.get("content", "")
                    if _total_extra_size <= _max_extra_chars:
                        break
                    if len(content) > 200:
                        trimmed = content[:200] + "\n...[内容已截断]"
                        reduction = len(content) - len(trimmed)
                        _total_extra_size -= reduction
                        msg["content"] = trimmed

            _round += 1

        # 兜底：如果全部轮次都是 tool_call，用无工具 LLM 生成最终回复
        if not final_reply:
            try:
                fallback_result = self._decide_engine.decide(
                    query=user_input,
                    conversation_id=conversation_id,
                    normalized_text=normalized.normalized_text,
                    pre_result=pre_result,
                    tools=None,  # 不给工具，强制纯文本
                    messages_extra=messages_extra,
                    platform_context=platform_context,
                )
                final_reply = fallback_result.reply or "抱歉，处理工具调用时遇到问题。"
            except Exception:
                final_reply = "抱歉，处理工具调用时遇到问题。"
        
        # 清洗可能的 XML 工具标签残余
        final_reply = self._strip_tool_tags(final_reply)
        
        # 自主延续：如果回复末尾暗示还有任务未完成，自动续杯
        _continue_hints = ("继续检查", "接着看", "接下来", "再看看", "还需要", "让我继续", "让我再")
        if any(hint in final_reply[-40:] for hint in _continue_hints):
            continuation_prompt = f"上一轮你说：{final_reply}\n\n请继续完成你提到的工作，完成后给出最终报告。"
            try:
                continuation_result = self._decide_engine.decide(
                ...
                if continuation_result.reply and not continuation_result.tool_call:
                    final_reply = self._strip_tool_tags(continuation_result.reply)
            except Exception:
                pass
    ...
    def chat_async(
        ...
        **kwargs,
        ...
        # 构建 Skill/吏员管理上下文（让 LLM 知道 create_clerk + 搜/装/分 skill）
        skill_context_msg = self._build_skill_context_message()
        messages_extra = [skill_context_msg] if skill_context_msg else None
    ...
    def _load_clerks(self) -> None:
        ...
        for entry in sorted(clerks_dir.iterdir()):
            ...
            try:
                result = _start_clerk(clerk_id, str(clerks_dir), registry=self.clerk_registry)
    ...
    def steward_resume_task(self, task_id: str) -> Optional[Dict[str, Any]]:
        """管家：恢复中断的任务（断点续传）

        已完成的子任务不会重复执行，只重新运行 pending/failed 的子任务。

        Args:
            task_id: 管家任务ID

        Returns:
            恢复后的任务信息，或 None（任务不存在或已完成）
        """
        task = self._steward.resume_task(task_id)
        if task is None:
            return None
        return {
        ...
        return {
            **memory_stats,
            "conversation": {
                ...
                "recent_pre_review_count": len(self._recent_pre_reviews),
            },
        }
