...
class DecideEngine:
    ...
    def __init__(
        ...
        if main_model is None:
            main_model = os.environ.get("SHIYI_MAIN_LLM_MODEL", "deepseek-v4-pro")
    ...
    def decide(
        ...
        if self.llm_available:
            ...
            return self._decide_without_llm(
                query=query,
                intent_result=intent_result,
                fragments=fragments,
            )
    ...
    def decide_stream(
        ...
        # 5-8. 流式 LLM 调用 + Fragment 提取存储
        if not self.llm_available:
            result = self._decide_without_llm(
                query=query,
                intent_result=intent_result,
                fragments=fragments,
            )
    ...
    def _decide_without_llm(
        ...
        # 简单摘要回复
        if fragments:
            top_fact = fragments[0].get("fact_kernel", "")
            reply = f"根据记忆：{top_fact}"
        else:
            reply = "当前没有相关记忆"
