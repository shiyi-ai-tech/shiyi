...
def validate_clerk_config(config: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    # 必填字段
    required = ["clerk_id", "name", "version", "description"]
...
class ClerkCreator:
    ...
    def create_non_interactive(
        ...
        新吏员不再自带工具——工具由全局共享池提供。
        自动复制主 LLM 配置到 llm_config。

        Args:
            name: 吏员名称
            desc: 吏员描述
            api_keys: 需要的API Key名称列表（如 ["DEEPSEEK_API_KEY"]）
            model_name: LLM模型名称（空则自动使用主 LLM 配置）
            base_url: API base URL（空则自动使用主 LLM 配置）
            provider: LLM provider（空则自动使用主 LLM 配置）
            data_policy: 数据边界
            version: 版本号
        """
        if not name:
        ...
        # 自动读取主 LLM 配置
        main_llm_config = self._get_main_llm_config()
        ...
        config = {
            ...
            # 自动继承主 LLM 配置（用户未指定时）
            "llm_config": {
                "provider": provider or main_llm_config.get("provider", DEFAULT_LLM_PROVIDER),
                "model": model_name or main_llm_config.get("model", DEFAULT_LIGHT_LLM_MODEL),
                "base_url": base_url or main_llm_config.get("base_url", DEFAULT_LLM_BASE_URL),
                "max_tokens": 2048,
                "temperature": 0.3,
            },
        }
    ...
    def _get_main_llm_config(self) -> Dict[str, str]:
        """读取主 LLM 配置（从环境变量或 .env 文件）
        ...
        return {
            ...
            "model": os.environ.get("SHIYI_MAIN_LLM_MODEL", ""),
            "base_url": os.environ.get("SHIYI_MAIN_API_BASE", ""),
        }
    ...
    def create_from_skill(
        ...
        config = {
            ...
            "skills": [skill_name],
            "llm_config": {
                "provider": main_llm_config.get("provider", DEFAULT_LLM_PROVIDER),
                "model": main_llm_config.get("model", DEFAULT_LIGHT_LLM_MODEL),
                "base_url": main_llm_config.get("base_url", DEFAULT_LLM_BASE_URL),
                "max_tokens": 2048,
                "temperature": 0.3,
            },
        }
    ...
    def _generate_worker(self, config: Dict[str, Any]) -> str:
        """生成worker.py内容 — 包含记忆代理工具"""
        tools = list(config.get("tools", []))  # 复制，避免修改原 config
        tool_names = [t["name"] for t in tools]

        # 始终追加记忆代理工具（吏员自身能力，非全局工具）
