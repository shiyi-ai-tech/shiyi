...
# shiyi+h: Hook 已被 ClerkRegistry 统一管理（Steward 不重复持有）
...
class SubTask:
    ...
    def _set_result(self, result: Optional[Dict[str, Any]], error: Optional[str] = None) -> None:
        """Thread-safe result + state update."""
        with self._lock:
            self.result = result
            self.error = error
            self.state = "done" if result is not None and error is None else "failed"
...
class StewardTask:
    ...
    _recovery_count: int = 0   # 自动恢复次数，防止 crash-loop
...
class Steward:
    ...
    def __init__(
        ...
        # shiyi+h: 权限 + Hook 已由 ClerkRegistry 统一管理（Steward 不重复持有）
        ...
        # 状态持久化
        self._state_dir = Path.home() / ".shiyi" / "data" / "steward"
    ...
    def execute(self, task: StewardTask) -> StewardTask:
        ...
        事件驱动并行调度：ready 列表中的子任务并发提交到线程池，
        不阻塞等待。主循环轮询已提交任务的状态，完成后解锁下游。
        ...
        while not self._all_terminal(task):
            # 提交所有 ready 子任务（不阻塞等待）
            ready = self._get_ready(task)
            for st in ready:
                self._submit_sub(task, st)
                    ...
                    for s in task.sub_tasks.values():
                        with s._lock:
                            if s.state == "pending":
                                pending.append(s)
                    if pending:
                        logger.error("Steward task %s: deadlock detected, %d pending tasks", task.task_id, len(pending))
                        for s in pending:
                            with s._lock:
                                s.state = "failed"
                                s.error = "死锁：依赖无法满足"
            ...
            # 等待任一 running 子任务完成（Event 通知，比轮询高效）
            running_subs = [s for s in task.sub_tasks.values()
                           if not s.is_terminal]
            if running_subs:
                # 等待最早可能完成的子任务的 event，最多 0.5s
                for s in running_subs:
                    s._done_event.wait(timeout=0.1)
                    if s.is_terminal:
                        break
        ...
        if self._aggregate_fn:
            try:
                ...
                task.summary = "; ".join(
                    (st.result.get("result", "") if isinstance(st.result, dict) else str(st.result))[:100]
                    for st in task.sub_tasks.values() if st.result
                )
    ...
    def _execute_pipeline_stage(self, stage: PipelineStage, upstream: Dict[str, Any]) -> Dict[str, Any]:
        ...
        使用 _submit_sub + 轮询等待完成，与 execute() 统一执行模式。
        ...
        while time.time() < deadline:
            if sub.is_terminal:
                break
            sub._done_event.wait(timeout=0.5)

        with sub._lock:
            return sub.result or {"result": sub.error or "done"}
    ...
    def _get_ready(self, task: StewardTask) -> List[SubTask]:
        ...
        for st in task.sub_tasks.values():
            ...
            # 检查所有依赖是否已完成
            deps_met = all(
                task.sub_tasks.get(did) and task.sub_tasks[did].is_terminal and task.sub_tasks[did].state == "done"
    ...
    def _submit_sub(self, task: StewardTask, st: SubTask) -> None:
        """提交子任务到线程池（非阻塞）

        替代旧 _execute_sub 的阻塞模式。提交后立即返回，
        线程池执行完成后通过回调更新 SubTask 状态。
        """
        with st._lock:
            st.state = "running"
        ...
        try:
            ...
            # 启动后台监控线程，等待执行完成后更新 SubTask 状态
            threading.Thread(
                target=self._watch_sub,
                args=(task.task_id, st.sub_id, st.submitted_task_id),
                daemon=True,
            ).start()
            logger.info("Sub-task %s submitted (executor task %s)", st.sub_id, st.submitted_task_id)
        except Exception as e:
            with st._lock:
                st.state = "failed"
                st.error = str(e)
                st.completed_at = time.time()
            st._done_event.set()
            logger.error("Steward sub-task %s/%s submit failed: %s", task.task_id, st.sub_id, e)

    def _watch_sub(self, task_id: str, sub_id: str, executor_task_id: str, timeout: float = 300.0) -> None:
        """后台等待子任务完成，更新 SubTask 状态
        ...
        while time.time() - start < timeout:
            tt = self._executor._tracker.get(executor_task_id)
            if tt is None:
                st._done_event.wait(timeout=0.5)
                continue
            status = tt.get("status", "")
            if status == "completed":
                result = tt.get("result", {})
                if isinstance(result, dict) and result.get("_proxy"):
                    result = self._registry._execute_proxy_tool(
                        result.get("tool", ""),
                        result.get("params", {}),
                    )
                with st._lock:
                ...
                self._persist_task(task)
                logger.info("Sub-task %s completed", sub_id)
                return
            if status == "failed":
                with st._lock:
                    st.state = "failed"
                ...
                st._done_event.set()
                self._persist_task(task)
                logger.error("Sub-task %s failed: %s", sub_id, st.error)
                return
            st._done_event.wait(timeout=0.3)
        ...
        st._done_event.set()
        self._persist_task(task)
        logger.error("Sub-task %s timed out", sub_id)
    ...
    def _persist_task(self, task: StewardTask) -> None:
        """将任务状态序列化到磁盘"""
        with self._persistence_lock:
            try:
                data = {
                    ...
                    "_recovery_count": getattr(task, '_recovery_count', 0),
                    "sub_tasks": {},
                }
                for sid, st in task.sub_tasks.items():
                    with st._lock:
                        data["sub_tasks"][sid] = {
                            "sub_id": st.sub_id,
                            ...
                            "result": st.result,
                            "error": st.error,
                            "submitted_task_id": st.submitted_task_id,
    ...
    _MAX_RECOVERY_COUNT = 3  # 最大自动恢复次数，超过后标记为 failed 不再自动恢复

    def _recover_tasks(self) -> None:
        """启动时从磁盘恢复未完成的任务

        自动恢复只重置状态，不自动重新执行（避免 crash-loop）。
        需要通过 resume_task() 手动恢复执行。
        超过 _MAX_RECOVERY_COUNT 次自动恢复的任务标记为 failed。
        """
        if not self._state_dir.exists():
            return
        recovered = 0
        for path in self._state_dir.glob("stew-*.json"):
            try:
                ...
                if data_version > 1:
                    logger.warning("Task %s has future version %d, attempting recovery anyway", task_id, data_version)

                # 恢复计数：防止 crash-loop
                recovery_count = data.get("_recovery_count", 0)
                ...
                # 保留恢复计数
                task._recovery_count = recovery_count

                for sid, sd in data.get("sub_tasks", {}).items():
                    st = SubTask(
                        sub_id=sd["sub_id"],
                        description=sd["description"],
                        clerk_id=sd["clerk_id"],
                        tool_name=sd["tool_name"],
                        tool_params=sd.get("tool_params", {}),
                        state=sd["state"],
                        depends_on=sd.get("depends_on", []),
                        dependents=sd.get("dependents", []),
                        result=sd.get("result"),
                        error=sd.get("error"),
                        started_at=sd.get("started_at", 0.0),
                        completed_at=sd.get("completed_at", 0.0),
                    )
                    task.sub_tasks[sid] = st

                self._tasks[task_id] = task
                recovered += 1

                # 如果任务仍在 running 状态（进程中断），标记为需恢复
                if task.state == "running":
                    if recovery_count >= self._MAX_RECOVERY_COUNT:
                        # 超过最大恢复次数，标记为 failed
                        task.state = "failed"
                        task.summary = f"自动恢复次数超过上限 ({self._MAX_RECOVERY_COUNT})，请手动检查"
                        for s in task.sub_tasks.values():
                            if s.state == "running":
                                with s._lock:
                                    s.state = "failed"
                                    s.error = "自动恢复次数超限"
                        logger.warning("Task %s: recovery count exceeded, marked as failed", task_id)
                        self._persist_task(task)
                    else:
                        # 只重置状态，不自动重新执行
                        running_subs = [s for s in task.sub_tasks.values() if s.state == "running"]
                        for s in running_subs:
                            with s._lock:
                                s.state = "pending"
                                s.started_at = 0.0
                                s.error = "进程中断后待恢复"
                        task.state = "ready"
                        task._recovery_count = recovery_count + 1
                        logger.info("Recovered task %s (count %d/%d) with %d sub-tasks pending resume",
                                    task_id, recovery_count + 1, self._MAX_RECOVERY_COUNT, len(running_subs))
                        self._persist_task(task)

            except Exception as e:
                logger.error("Failed to recover task from %s: %s", path, e)

        if recovered:
            logger.info("Steward recovered %d tasks from disk", recovered)

    def resume_task(self, task_id: str) -> Optional[StewardTask]:
        """恢复一个未完成的任务（断点续传）

        将之前中断的任务重新执行。已完成的子任务不会重复执行。
        """
        task = self._tasks.get(task_id)
        if task is None:
            return None
        if task.state in ("done", "running"):
            return task

        # 只有 ready/failed 状态的任务可以恢复
        logger.info("Resuming task %s (%d sub-tasks, state=%s)", task_id, len(task.sub_tasks), task.state)
