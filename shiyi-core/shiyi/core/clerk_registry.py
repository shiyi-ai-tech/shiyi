...
- 全局工具：register_global_tool() 将工具注册为全局共享，所有吏员可用
- 执行：自动路由到正确的吏员或全局/直注册处理器
- 工具同名：多吏员可共享同名工具，_tool_clerk 存列表

接口对上层透明 — engine.talk() 调用 get_schemas() + execute() 即可。
"""
...
# shiyi+h: 权限引擎 + Hook 框架
from shiyi.core.permissions import PermissionEngine
from shiyi.core.hooks import HookManager, path_guard, command_guard, audit_log
...
class ClerkRegistry:
    ...
    def __init__(self):
        ...
        # 全局工具来源追踪：tool_name → source_clerk_id
        self._global_tool_sources: Dict[str, str] = {}
        # shiyi+h: 一站式权限 + Hook（确保不经 Steward 的直接调用也被拦截）
        self._permissions = PermissionEngine()
        self._hooks = HookManager()
        self._hooks.register_pre_wildcard(path_guard)
        self._hooks.register_pre_wildcard(command_guard)
        self._hooks.register_post_wildcard(audit_log)
        # 异步执行线程池（复用，避免每次新建线程）
        from concurrent.futures import ThreadPoolExecutor
        self._async_pool = ThreadPoolExecutor(max_workers=8)
    ...
    def unregister_clerk(self, clerk_id: str) -> bool:
        ...
        已提升为全局的工具（在 _global_tool_sources 中）不会被删除。
        """
    ...
    def register_global_tool(
        ...
        schema: Dict[str, Any],
        handler: Callable[[Dict[str, Any]], Dict[str, Any]],
        source_clerk: Optional[str] = None,
    ) -> None:
        """注册全局共享工具

        全局工具对所有吏员可用，删除吏员不影响全局工具。
        通常用于 clerk-default 的基础工具（bash, file_read 等）。

        Args:
            name: 工具名
            schema: Function Calling schema（含 type, function 字段）
            handler: 调用处理器
            source_clerk: 来源吏员 ID（可选，用于追踪）
        """
        self._direct_tools[name] = {
            "schema": schema,
            "handler": handler,
        ...
        if source_clerk:
            self._global_tool_sources[name] = source_clerk
        logger.info("Registered global tool: %s (source: %s)", name, source_clerk or "direct")

    def promote_clerk_tools_to_global(self, clerk_id: str) -> int:
        """将吏员的所有工具提升为全局共享

        将指定吏员的工具从 _tool_clerk 移至 _direct_tools，
        使其成为所有吏员可用的全局工具。删除吏员时全局工具不受影响。
        ...
        Returns:
            提升的工具数量
        """
        clerk_data = self._clerks.get(clerk_id)
        if not clerk_data:
            logger.warning("Cannot promote tools: clerk %s not found", clerk_id)
            return 0

        worker = clerk_data["worker"]
        tools = clerk_data["tools"]
        count = 0
        ...
        for tool in tools:
            ...
            # handler 委托到原吏员的 worker
            handler = lambda args, _name=tname: worker.execute(_name, args)

            # 注册为全局工具
            self._direct_tools[tname] = {
                "schema": schema,
                "handler": handler,
            }
            self._global_tool_sources[tname] = clerk_id

            # 从 _tool_clerk 中移除该吏员的映射
            if tname in self._tool_clerk:
                try:
                    self._tool_clerk[tname].remove(clerk_id)
                    if not self._tool_clerk[tname]:
        ...
        logger.info("Promoted %d tools from clerk %s to global", count, clerk_id)
        return count
    ...
    def get_schemas(self) -> List[Dict[str, Any]]:
        ...
        # 直注册工具（也需去重，防止与吏员工具同名）
        for t in self._direct_tools.values():
            tname = t["schema"]["function"]["name"]
            if tname in seen:
                continue
            seen.add(tname)
    ...
    def execute(
        ...
        """
        # shiyi+h: 一站式权限检查 + PreHook
        perm = self._permissions.check(name, clerk_id)
        if perm.denied:
            return {"success": False, "result": "", "error": perm.reason}
        if perm.need_confirm:
            return {"success": False, "result": "", "error": perm.reason}

        # PreHook（调用工具前拦截/修改参数）
        hook_result = self._hooks.run_pre(name, arguments)
        if hook_result.blocked:
            return {"success": False, "result": "", "error": hook_result.reason}
        if hook_result.modified_params:
            arguments = hook_result.modified_params
        ...
        if clerk_ids is not None:
            ...
            try:
                ...
                if isinstance(result, dict) and result.get("_proxy"):
                    ...
                    result = self._execute_proxy_tool(tool_name, params)

                # PostHook（执行后过滤/增强结果）
                result = self._hooks.run_post(name, arguments, result)
                return result
    ...
    def list_tools(self) -> List[str]:
        """列出所有可用工具名（含全局工具）"""
        all_tools = set(self._tool_clerk.keys()) | set(self._direct_tools.keys())
    ...
    def tool_count(self) -> int:
        return len(set(self._tool_clerk.keys()) | set(self._direct_tools.keys()))

    def __contains__(self, name: str) -> bool:
        return name in self._tool_clerk or name in self._direct_tools

    def is_global_tool(self, tool_name: str) -> bool:
        """检查工具是否为全局共享工具"""
        return tool_name in self._global_tool_sources
