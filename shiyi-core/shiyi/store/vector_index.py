...
class VectorIndex:
    ...
    def get_vector(self, fragment_id: str) -> Optional[List[float]]:
        ...
        if not self._index:
            return None
        internal_id = self._external_to_id.get(fragment_id)
        if internal_id is None or internal_id in self._deleted_ids:
            return None
        try:
            return self._index.get_items([internal_id])[0].tolist()
        except Exception:
            return None
