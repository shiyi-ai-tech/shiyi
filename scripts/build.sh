...
if ! command -v shiyi &>/dev/null; then
    ...
    pip install -e "$PROJECT_DIR/shiyi-common" -e "$PROJECT_DIR/shiyi-providers" -e "$PROJECT_DIR/shiyi-core" -e "$PROJECT_DIR/shiyi-shell" -q
fi
...
# CLI 验证
shiyi --version 2>&1 | grep -q "$VERSION" || { echo "❌ CLI --version failed"; exit 1; }
echo "  ✓ shiyi --version ($VERSION)"
