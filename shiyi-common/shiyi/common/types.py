...
class SceneShell:
    ...
    location: str = ""
    activation_level: float = 0.5  # 激活水平 0~1（v260530.4 新增）
    source: str = ""               # 来源：对话/工具/系统（v260530.4 新增）
...
class IntentType(str, Enum):
    """意图类型 — 运行时LLM产出+内部路由统一枚举

    由 IntentEngine.analyze() 的 LLM prompt 产出。
    可用于意图路由、检索策略选择等。
    """
    query = "query"          # 查询类：问事实、问信息、问知识
    ...
    # 以下为内部路由专用（非LLM产出）
    entity = "entity"        # 实体查询（路由用）
    fact = "fact"            # 事实查询（路由用）
    greeting = "greeting"    # 问候（路由用）
...
class PreReviewResult:
    """预审结果（v260530: 替代 IntentResult，只做区分+拆分+情感）

    预审 flash LLM 不判断意图、不分类、不短路、不路由。
    只产出：要不要检索、搜什么、情感如何。
    """
    need_retrieval: bool = True
    search_terms: List[str] = field(default_factory=list)  # 扁平关键词列表
    emotion_valence: float = 0.0                           # -1.0 ~ +1.0
    emotion_arousal: float = 0.0                           # 0.0 ~ 1.0
    emotion_dominant: str = "中性"                          # 主导情感标签
    raw_output: str = ""                                   # 原始LLM输出


@dataclass
class IntentResult:
    """意图解析结果（DEPRECATED: 由 PreReviewResult 替代，保留兼容旧调用方）"""
