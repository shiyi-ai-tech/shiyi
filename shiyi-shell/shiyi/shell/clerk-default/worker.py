...
# 默认工作目录 — 史佚进化（shiyi-se）：限制在 ~/.shiyi/，可访问自身源码
DEFAULT_WORKSPACE = Path.home() / ".shiyi"
...
class FileReadTool:
    ...
    name = "file_read"
    description = "读取文件内容或列出目录。安全限制在 ~/.shiyi/ 项目目录内。"
...
class FileWriteTool:
    ...
    name = "file_write"
    description = "写入内容到文件。安全限制在 ~/.shiyi/ 项目目录内。"
...
_COMMAND_BLACKLIST = [
    ...
    r"parted\s",
    # 数据保护：禁止删除用户数据
    r"rm\s+.*\.shiyi/(data|skills|clerks|\.env|\.backup)",
    r"rmdir\s+.*\.shiyi",
    r">\s*.*\.shiyi/\.env",
    r"mv\s+.*\.shiyi/(data|skills|clerks|\.env)\s+/dev/null",
]
...
TOOL_REGISTRY: Dict[str, Any] = {
    ...
    # 增强版工具
    "bash": BashTool,
    "enhanced_file_read": EnhancedFileReadTool,
    "enhanced_file_write": EnhancedFileWriteTool,
...
class ClerkWorker:
    ...
    def _register_local_handlers(self) -> None:
        ...
        self._handlers = {
            ...
            # 增强版工具
            "bash": lambda params: BashTool.execute(params, self.workspace),
            "enhanced_file_read": lambda params: EnhancedFileReadTool.execute(params, self.workspace),
            "enhanced_file_write": lambda params: EnhancedFileWriteTool.execute(params, self.workspace),
            "file_edit": lambda params: FileEditTool.execute(params, self.workspace),
            "file_glob": lambda params: FileGlobTool.execute(params, self.workspace),
            "file_grep": lambda params: FileGrepTool.execute(params, self.workspace),
            "enhanced_web_search": lambda params: EnhancedWebSearchTool.execute(params, self.workspace),
            "web_fetch": lambda params: WebFetchTool.execute(params, self.workspace),
            "ask_user": lambda params: AskUserTool.execute(params, self.workspace),
        }
    ...
    def run_agent_loop(
        ...
        if timeout is None:
            timeout = agent_cfg.get("default_timeout", 600)
...
5. 遇到错误时分析原因并尝试替代方案，不要直接放弃
6. 无法完成任务时用「FINAL:」说明原因和已完成的进度
7. 如果一个工具调用失败，可以重试或换一种方式
...
{skills_context}"""
        ...
        consecutive_no_tool = 0  # 连续无工具调用的次数
        max_no_tool = 4          # 连续N次无工具调用则视为完成
        llm_retry_count = 0
        max_llm_retries = 2      # LLM 调用连续失败最大重试次数
        last_response = ""       # 保留最后一次有效回复
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            if elapsed > timeout:
                ...
                if last_response:
                    return {
                        "success": False,
                        "result": last_response[:2000],
                        "iterations": iteration,
                        "error": f"执行超时 ({timeout}s)，返回部分结果",
                        "trace": trace,
                    }
                return {
                    "success": False,
            ...
            for retry in range(max_llm_retries + 1):
                ...
                if retry < max_llm_retries:
                    logger.warning("LLM call failed, retrying (%d/%d)", retry + 1, max_llm_retries)
            ...
            if response is None:
                ...
                if llm_retry_count >= 3:
                    return {
                        "success": False,
                        "result": last_response[:2000] if last_response else "",
                        "iterations": iteration,
                        "error": "LLM 连续调用失败",
                        "trace": trace,
                    }
            ...
            if self._is_completion_signal(stripped):
                final_result = self._extract_final_result(stripped)
                return {
                    "success": True,
                    "result": final_result,
                    "iterations": iteration,
            ...
            if tool_call:
                ...
                # 工具执行（带重试）
                result = self._execute_tool_with_retry(tool_name, tool_params, max_retries=1)
                ...
                if result.get("success"):
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            f"工具 {tool_name} 执行失败：{result.get('error', '未知错误')}\n"
                            f"请分析原因并尝试其他方法。"
                        ),
                    })
            else:
                ...
                # 连续多次无工具调用，可能 LLM 认为任务已完成但没用 FINAL:
                if consecutive_no_tool >= max_no_tool:
                    # 检查是否有明确的完成信号，否则视为完成
                    has_completion = self._is_completion_signal(stripped)
                    if has_completion or consecutive_no_tool >= max_no_tool + 1:
                        return {
                            "success": True,
                            "result": stripped[:2000],
                            "iterations": iteration,
                            "error": "",
                            "trace": trace,
                        }
                ...
                if consecutive_no_tool == 1:
                    messages.append({
                        "role": "user",
                        "content": "请继续。如果任务已完成，用「FINAL:」汇报结果。如果需要使用工具，请按JSON格式调用。",
                    })
                elif consecutive_no_tool == 2:
                    messages.append({
                        "role": "user",
                        "content": "请使用工具完成任务。如果确实已完成，请用「FINAL:」开头汇报最终结果。",
                    })
                else:
                    messages.append({
                        "role": "user",
                        "content": "请继续执行。如果已完成，请用「FINAL:」开头汇报最终结果。",
                    })

            # 防止消息列表无限膨胀（保留 system + 最近 20 轮）
            if len(messages) > 42:
    ...
    def _is_completion_signal(self, text: str) -> bool:
        ...
        return any(text.startswith(m) for m in completion_markers)

    def _extract_final_result(self, text: str) -> str:
        """从完成信号中提取最终结果"""
        for marker in ("FINAL:", "final:", "Final:", "DONE:"):
            if text.startswith(marker):
                return text[len(marker):].strip()
        for marker in ("任务完成", "任务已完成", "已完成", "TASK COMPLETE"):
            if text.startswith(marker):
                return text[len(marker):].strip()
        return text

    def _execute_tool_with_retry(
        self, tool_name: str, params: Dict[str, Any], max_retries: int = 1,
    ) -> Dict[str, Any]:
        """执行工具调用，失败时重试"""
        last_result = None
        for attempt in range(max_retries + 1):
            ...
            if attempt < max_retries:
                ...
                logger.warning("Tool %s failed (attempt %d): %s", tool_name, attempt + 1, error_msg)
                time.sleep(1)
        return last_result
    ...
    def _parse_tool_call(self, response: str) -> Optional[Dict[str, Any]]:
        """
        从 LLM 响应中解析工具调用 — 多格式容错

        支持的格式（按优先级）：
        1. Function Calling JSON: {"tool": "web_search", "params": {"query": "..."}}
        2. 带 tool/params 的 JSON（可能嵌在自然语言中）
        3. Markdown code block 中的 JSON
        4. XML 格式: <tool_call tool="web_search"><params>{"query":"..."}</params></tool_call >
        5. 自然语言前缀: "调用 web_search 工具，参数: {...}"
        6. 行内 JSON 片段: tool_name({...})
        ...
        # Strategy 1: 直接 JSON 解析（整个响应是一个 JSON 对象）
        try:
            parsed = json.loads(response.strip())
            if isinstance(parsed, dict):
                result = self._extract_tool_from_dict(parsed)
                if result:
                    return result
        except json.JSONDecodeError:
            pass

        # Strategy 2: Markdown code block 中的 JSON
        ...
        # Strategy 3: XML 格式 <tool_call tool="..."><params>...</params></tool_call >
        xml_match = re.search(
            r'<tool_call\s+tool\s*=\s*["\']([^"\']+)["\']\s*>(.*?)</tool_call\s*>',
        ...
        if xml_match:
            ...
            params = {}
            params_match = re.search(r'<params\s*>(.*?)</params\s*>', inner, re.DOTALL)
            if params_match:
                try:
        ...
        # Strategy 4: 自然语言前缀 — "调用 xxx 工具" / "使用 xxx" / "Call xxx"
        nl_match = re.search(
            r'(?:调用|使用|执行|Call|Use|Run)\s+(\w+)\s*(?:工具|tool)?\s*[，,：:]\s*(\{.*?\})',
            response, re.DOTALL | re.IGNORECASE,
        )
        if nl_match:
            tool_name = nl_match.group(1).strip()
            try:
                params = json.loads(nl_match.group(2).strip())
                if isinstance(params, dict):
                    return {"tool": tool_name, "params": params}
            except json.JSONDecodeError:
                pass

        # Strategy 5: 行内 JSON 片段 — tool_name({...}) 或 tool_name: {...}
        ...
        if inline_match:
            ...
            try:
                params = json.loads(inline_match.group(2).strip())
                if isinstance(params, dict) and tool_name in self._handlers:
                    return {"tool": tool_name, "params": params}
            except json.JSONDecodeError:
                pass

        # Strategy 6: 扫描响应中所有 JSON 对象，找含 tool/params 或匹配已知工具名的
        # 仅在 tool 名匹配已注册 handler 时才接受，防止误匹配数据展示 JSON
        # 额外保护：只匹配明确的 tool 调用格式，不匹配带 action/name 的普通 JSON
        for match in re.finditer(r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}', response, re.DOTALL):
            try:
                parsed = json.loads(match.group())
                if isinstance(parsed, dict):
                    result = self._extract_tool_from_dict(parsed)
                    if result and result["tool"] in self._handlers:
                        # 额外验证：要求必须包含 params 且非空，或者 tool 字段显式存在
                        has_explicit_tool = "tool" in parsed or "function" in parsed
                        has_params = bool(result.get("params"))
                        if has_explicit_tool or has_params:
                            return result
            except json.JSONDecodeError:
                pass
    ...
    def _extract_tool_from_dict(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """从 JSON dict 中提取工具调用，支持多种字段命名"""
        # 标准格式: {"tool": "...", "params": {...}}
        if "tool" in data and "params" in data:
            return {"tool": str(data["tool"]), "params": data["params"] if isinstance(data["params"], dict) else {}}

        # Function Calling 格式: {"name": "...", "arguments": {...}}
        if "name" in data and "arguments" in data:
            ...
            if isinstance(args, str):
                try:
                    ...
                    args = {}
            return {"tool": str(data["name"]), "params": args if isinstance(args, dict) else {}}

        # 带 function 包装: {"function": {"name": "...", "arguments": "..."}}
        ...
        if isinstance(func, dict) and "name" in func:
            ...
            if isinstance(args, str):
                try:
                    ...
                    args = {}
            return {"tool": str(func["name"]), "params": args if isinstance(args, dict) else {}}

        # action 格式: {"action": "tool_name", ...其他当params}
        # 仅匹配已注册的工具名，避免把 {"action": "click"} 这类普通 JSON 误判
        if "action" in data and isinstance(data["action"], str) and data["action"] in self._handlers:
            params = {k: v for k, v in data.items() if k != "action"}
            return {"tool": data["action"], "params": params}
