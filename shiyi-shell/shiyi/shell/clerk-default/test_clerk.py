...
def test_clerk_json_structure():
    ...
    try:
        ...
        all_known_tools = {
            "file_read", "file_write", "web_search", "run_command",
            "recall_memory", "search_conversations",
            "bash", "enhanced_file_read", "enhanced_file_write",
            "file_edit", "file_glob", "file_grep",
            "enhanced_web_search", "web_fetch", "ask_user",
        }
        caps = set(data.get("capabilities", []))

        unknown = caps - all_known_tools
        if unknown:
